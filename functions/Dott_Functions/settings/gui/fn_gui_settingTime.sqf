...
private _ctrlSlider = _controlsGroup controlsGroupCtrl 5140;
_ctrlSlider setVariable ["cba_settings_params", [_setting, _source]];
...
_ctrlSlider ctrlAddEventHandler ["SliderPosChanged", {
    ...
    (_ctrlSlider getVariable "cba_settings_params") params ["_setting", "_source"];
    _value = round _value;

    private _controlsGroup = ctrlParentControlsGroup _ctrlSlider;
    (_controlsGroup controlsGroupCtrl 5141) ctrlSetText ([floor (_value / 3600), 2] call CBA_fnc_formatNumber);
    (_controlsGroup controlsGroupCtrl 5142) ctrlSetText ([floor (_value / 60 % 60), 2] call CBA_fnc_formatNumber);
    (_controlsGroup controlsGroupCtrl 5143) ctrlSetText ([floor (_value % 60), 2] call CBA_fnc_formatNumber);
...

{
    ...
    _ctrlEdit setVariable ["cba_settings_params", [_setting, _source]];
    _ctrlEdit ctrlSetText ([_value, 2] call CBA_fnc_formatNumber);

    _ctrlEdit ctrlAddEventHandler ["KillFocus", {
        params ["_ctrlEdit"];
        (_ctrlEdit getVariable "cba_settings_params") params ["_setting", "_source"];

        private _controlsGroup = ctrlParentControlsGroup _ctrlEdit;
        private _ctrlSlider = _controlsGroup controlsGroupCtrl 5140;
        private _ctrlEditHours = _controlsGroup controlsGroupCtrl 5141;
        private _ctrlEditMinutes = _controlsGroup controlsGroupCtrl 5142;
        private _ctrlEditSeconds = _controlsGroup controlsGroupCtrl 5143;
...
_controlsGroup setVariable ["cba_settings_fnc_updateUI", {
    ...
    (_controlsGroup controlsGroupCtrl 5140) sliderSetPosition _value;
    (_controlsGroup controlsGroupCtrl 5141) ctrlSetText ([floor (_value / 3600), 2] call CBA_fnc_formatNumber);
    (_controlsGroup controlsGroupCtrl 5142) ctrlSetText ([floor (_value / 60 % 60), 2] call CBA_fnc_formatNumber);
    (_controlsGroup controlsGroupCtrl 5143) ctrlSetText ([floor (_value % 60), 2] call CBA_fnc_formatNumber);
