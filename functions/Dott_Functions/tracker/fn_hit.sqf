...
if (_hitEntity isKindOf "Man") exitWith { _hitEntity setVariable ["DOTT_lastHit", _lastHit, 2] };
...
{ if (alive _x) then { _x setVariable ["DOTT_lastHit", _lastHit, 2] } }
forEach (crew _hitEntity);

_hitEntity setVariable ["DOTT_lastHit", _lastHit, 2];
