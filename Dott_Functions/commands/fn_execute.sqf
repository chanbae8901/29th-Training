...
private _chatArr = [_this, 0, []] call BIS_fnc_param;

// Strip the leading command marker character.
_chatArr set [0, -1];
_chatArr = _chatArr - [-1];

private _separator = (toArray " ") select 0;
private _commandDone = false;
private _command = [];
private _argument = [];

// Split on first space: everything before is the command
// name, everything after is the argument string.
{
...
{
    // Check if command has been disabled by the server.
    ...
    // Round-restricted gate (non-admins during active round).
    if (
        pvpfw_chatIntercept_restrictedCommands find _command != -1
        && !_isAdmin
    ...
    // Log command usage unless it's in the no-log list.
