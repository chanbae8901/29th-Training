 ...
 *
 * Purpose:
 * Copies a diary record's text to clipboard, stripping the
 * "Copy to Clipboard" execute button from the output.
 * Requires ACE 3.
 *
 * Parameters:
...
private _allRoundRecords =
    player allDiaryRecords _subject;
private _idx =
    _allRoundRecords findIf
        { _x select 1 == _title };
private _recordText =
    (_allRoundRecords select _idx) select 2;

// Strip everything from the <execute> button onward.
_recordText =
