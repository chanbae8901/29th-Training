...
private _time =
    if (_eventTime isEqualType []) then
    {
        _eventTime select 0
    }
    else
...
(_time call _fn_formatTime) params
    ["_minutes", "_secondStr"];

private _eventString = "";

// ---------------------------------------------------------------
// Shared kill/destroy formatter for infantry and vehicle kills.
// Only the verb differs ("killed" vs "destroyed").
// ---------------------------------------------------------------
private _fn_formatKill =
{
    ...
    private _unitSide = [_unitIndex, _eventTime, _sides]
...
switch (_eventType) do
{
    ...
    case DELAY_KILL_NUM:
    {
        ...
        private _instigatorName =
            _names select _instigatorIndex;
        // Use hitTime for instigator side lookup.
        private _instigatorSide =
            [_instigatorIndex,
        ...
        (_time2 call _fn_formatTime) params
            ["_minutes2", "_secondStr2"];
        _eventString = format [
        ...
        if (_state) then
        {
            if (count _eventInfo > 2) then
            {
                ...
                _instigatorName =
                    [_instigatorName, _instigatorSide]
                        call
                    DOTT_tracker_fnc_colorNameWithSide;
                private _distance =
                    _eventInfo select 3;
                private _weapon =
                    _weapons select
                        (_eventInfo select 4);
                _eventString = format [
                    "%1:%2 - %3 unconscious by"
                    + " %4 [%5] from %6 meters.",
                    _minutes, _secondStr,
                    _unitName, _instigatorName,
                    _weapon, _distance
                ];
            }
            else
    ...
    case DELAY_ACE_CONSCIOUSNESS_NUM:
    {
        ...
        (_time2 call _fn_formatTime) params
            ["_minutes2", "_secondStr2"];
        _eventString = format [
            "%1:%2 - %3 finally unconscious by"
            + " %4 [%5] (%6:%7) from %8 meters. ",
            _minutes, _secondStr, _unitName,
            _instigatorName, _weapon,
            _minutes2, _secondStr2, _distance
        ];
