...
// Block until the previous round's diary has been written.
// Rounds are sent in order but remoteExec can deliver them
// out of sequence. This waitUntil serializes processing so
// diary entries always appear in the correct round order.
waitUntil
{
...
// ---------------------------------------------------------------
// Create diary subject on first use
// ---------------------------------------------------------------
...
// ---------------------------------------------------------------
// Generate all event strings (newest first for diary display)
// ---------------------------------------------------------------
...
// ---------------------------------------------------------------
// Filter to player-relevant events
// ---------------------------------------------------------------
private _playerEventIndexes =
    [_playerIndex, _events]
        call DOTT_tracker_fnc_findPlayerEvents;

// Remap indices to match the reversed _eventStrings order.
for "_i" from 0
...
// ---------------------------------------------------------------
// Write personal events diary record
// ---------------------------------------------------------------
...
private _killCounts =
    [_events, _sides] call
        DOTT_tracker_fnc_getKillCounts;
_title = format ["Round %1", _roundNum];
_text = [_killCounts, _names] call
    DOTT_tracker_fnc_killCountsToString;
_copyButton = format [
