...
/**
 * @description Changes the ready state of a side and checks if all
 *     sides are ready to start the round. If all sides become ready,
 *     initiates safe start or shortens an existing forced safe start.
 * @param {Side} _side - Which side to change ready state for.
 * @param {Boolean} _isReady - Desired ready state.
 * @return {Number} 2 if side already in requested state, 1 if round
 *     already started, 0 otherwise.
 * @example [west, true] call DOTT_round_fnc_manageReady;
 */
